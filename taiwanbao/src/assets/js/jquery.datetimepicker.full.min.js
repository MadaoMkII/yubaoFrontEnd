                ...
                if ($.type(opt) === 'string') {
                    switch (opt) {
                        ...
                        case 'reset':
                            ...
                            break;
                        case 'getValue':
                            result = datetimepicker.getValue();
                    }
                } else {
                    datetimepicker
                        .setOptions(opt);
...
(function() {
    ...
    Date.createNewFormat = function(format) {
        ...
        var special = false;
        var ch = "";
        for (var i = 0; i < format.length; ++i) { ch = format.charAt(i);
            if (!special && ch == "\\") { special = true; } else {
                if (special) { special = false;
                    code += "'" + String.escape(ch) + "' + "; } else { code += Date.getFormatCode(ch); } } }
    ...
    Date.createParser = function(format) {
        ...
        for (var i = 0; i < format.length; ++i) { ch = format.charAt(i);
            if (!special && ch == "\\") { special = true; } else {
                if (special) { special = false;
                    regex += String.escape(ch); } else { obj = Date.formatCodeToRegex(ch, currentGroup);
                    currentGroup += obj.g;
                    regex += obj.s;
